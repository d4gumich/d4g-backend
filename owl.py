...
SIMILARITY_API_URL = os.environ.get(
    "SIMILARITY_API", "https://similarity-api-559650505418.us-central1.run.app/similarity"
)
...
def ask():
    try:
        ...
        try:
            ...
            return jsonify({"error": f"Similarity API error: {e!s}"}), 500

        if not similar_docs:
            return jsonify(
                {"answer": "I couldn't find any relevant documents to answer your question.", "documents": []}
            )
        ...
        return jsonify({"answer": answer, "documents": similar_docs})
